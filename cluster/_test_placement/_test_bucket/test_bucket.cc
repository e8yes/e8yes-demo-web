...
std::unordered_set<e8::ClusterTreeNodeLabel> WillSelect(e8::ResourceDescriptor const &resource,
    ...
    for (unsigned f = 0; f < max_failure_tolerance; ++f) {
        std::optional<e8::ClusterTreeNodeLabel> child = bucket->Select(resource, f);
        if (child.has_value()) {
...
bool GeneralTest(e8::BucketInterface *bucket, bool test_capability_distribution) {
    ...
    coral_hungry_resource.key = "e";
    coral_hungry_resource.rank = 0;
    coral_hungry_resource.num_replicas = 2;

    // The bucket should be empty at this moment.
    std::optional<e8::ClusterTreeNodeLabel> child =
        bucket->Select(cpu_hungry_resource, /*num_failures=*/0);
    TEST_CONDITION(!child.has_value());
    child = bucket->Select(ram_hungry_resource, /*num_failures=*/0);
    TEST_CONDITION(!child.has_value());
    child = bucket->Select(storage_hungry_resource, /*num_failures=*/0);
    TEST_CONDITION(!child.has_value());
    child = bucket->Select(coral_hungry_resource, /*num_failures=*/0);
    TEST_CONDITION(!child.has_value());
    child = bucket->Select(dont_care_resource, /*num_failures=*/0);
    TEST_CONDITION(!child.has_value());
    ...
    high_performance_cpu_capabilities.set_ram(1.0f);
    ...
    high_ram_capacity_capabilities.set_cpu(1.0f);
    ...
    high_storage_capacity_capabilities.set_ram(1.0f);
    ...
    high_storage_capacity_capabilities.set_cpu(0.0f);
    high_storage_capacity_capabilities.set_ram(1.0f);
    high_storage_capacity_capabilities.set_storage(0.0f);
    high_storage_capacity_capabilities.set_coral(4.0f);

    // Adds nodes to the bucket, and checks that the bucket does not allow double addings/overrides.
    bool added = bucket->AddChild(high_performance_cpu_node, high_performance_cpu_capabilities);
    TEST_CONDITION(added);
    added = bucket->AddChild(high_ram_capacity_node, high_ram_capacity_capabilities);
    TEST_CONDITION(added);
    added = bucket->AddChild(high_storage_capacity_node, high_storage_capacity_capabilities);
    TEST_CONDITION(added);
    added = bucket->AddChild(coral_node, coral_capabilities);
    TEST_CONDITION(added);

    e8::WeightedCapabilities bad_capabilities;
    added = bucket->AddChild(high_performance_cpu_node, bad_capabilities);
    TEST_CONDITION(!added);
    added = bucket->AddChild(high_ram_capacity_node, bad_capabilities);
    TEST_CONDITION(!added);
    added = bucket->AddChild(high_storage_capacity_node, bad_capabilities);
    TEST_CONDITION(!added);
    added = bucket->AddChild(coral_node, bad_capabilities);
    TEST_CONDITION(!added);

    // Checks if the bucket distribute resource according to the hardware capability setting and
    // requirement.
    if (test_capability_distribution) {
        child = bucket->Select(cpu_hungry_resource, /*num_failures=*/0);
        TEST_CONDITION(child.has_value());
