...
void DropSchema(e8::ConnectionInterface *conn) {
    ...
    conn->run_update(dropCardTable, e8::ConnectionInterface::QueryParams());
    conn->run_update(dropUserTable, e8::ConnectionInterface::QueryParams());
}
...
void CreateSchema(e8::ConnectionInterface *conn) {
    std::string create_user_table = "CREATE TABLE QueryRunnerTestUser("
                                    ...
                                    "   PRIMARY KEY (id))";
    conn->run_update(create_user_table, e8::ConnectionInterface::QueryParams());
    ...
    conn->run_update(create_card_table, e8::ConnectionInterface::QueryParams());
}
...
void sql_runner_test::insert_then_query_test() {
    ...
    *user.user_name.value_ptr() = "user0";
    ...
    CreditCard card0;
    *card0.id.value_ptr() = 10;
    *card0.card_number.value_ptr() = "1234";
    *card0.user_id.value_ptr() = 1;

    CreditCard card1;
    *card1.id.value_ptr() = 11;
    *card1.card_number.value_ptr() = "2234";
    *card1.user_id.value_ptr() = 1;
    ...
    std::vector<std::tuple<User, CreditCard>> results = e8::Query<User, CreditCard>(
        e8::SqlQueryBuilder().query_piece(
